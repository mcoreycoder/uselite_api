...
// console.log("req.app.locals.usersCollection",req.app.locals.usersCollection) //is undefind here
...
// login - (had to add .env JWT_KEY so the makeToken() would work)
// Postman test 7/25/20 - working (added handling for null return)
router.post('/login', function (req, res, next) {

    console.log("login req.body:",req.body)
    db.findOne({ query: { email: req.body.email }, resource: req.baseUrl.slice(1) })
        .then((user) => {
            if(user!==null){
...
router.post('/auth/verifytoken', verifyToken, function (req, res, next) {
    ...
    db.findOne({ query: { email: req.email }, resource: req.baseUrl.slice(1) })
